...
if(request.status === 200){
	...
	for(var i = 0; i < json.meats.length; i++){
		...
		html += '<input type="radio" name="meat" value="'+json.meats[i].name+'" >'+json.meats[i].name;
                if(json.meats[i].name.indexOf("Beef") > -1){
                    html += '<img class = "MenuPics" id="'+json.meats[i].name+'_Pic'+'" src="img/Beef.png">';
                }
                else{
...
if(request.status === 200){
	...
	for(var i = 0; i < json.buns.length; i++){
		...
		html += '<input type="radio" name="bun" value="'+json.buns[i].name+'" >'+json.buns[i].name;
                html += '<img class = "MenuPics" src="img/' + json.buns[i].name+'.png" id="'+json.buns[i].name+'">';
		bunPrices.push(temp);
	}
...
if(request.status === 200){
	...
	for(var i = 0; i < json.cheeses.length; i++){
		...
		html += '<input type="radio" name="cheese" value="'+json.cheeses[i].name+'" >'+json.cheeses[i].name;
                html += '<img class = "MenuPics" src="img/' + json.cheeses[i].name+'.png" id="'+json.cheeses[i].name+'">';
...
if(request.status === 200){
	...
	for(var i = 0; i < json.toppings.length; i++){
		...
		html += '<input type="checkbox" name="topping" value="'+json.toppings[i].name+'" >'+json.toppings[i].name;
                html += '<img class = "MenuPics" src="img/' + json.toppings[i].name+'.png" id="'+json.toppings[i].name+'">';
...
if(request.status === 200){
	...
	for(var i = 0; i < json.sauces.length; i++){
		...
		html += '<input type="checkbox" name="sauce" value="'+json.sauces[i].name+'" >'+json.sauces[i].name;
                html += '<img class = "MenuPics" src="img/' + json.sauces[i].name+'.png" id="'+json.sauces[i].name+'">';
...
if(request.status === 200){
	...
	for(var i = 0; i < json.sides.length; i++){
		...
		html += '<input type="radio" name="side" value="'+json.sides[i].name+'" >'+json.sides[i].name;
                html += '<img class = "MenuPics" src="img/' + json.sides[i].name+'.png" id="'+json.sides[i].name+'">';
...
for (var i = 0; i < meats.length; ++i) {
  ...
  var idNumberMeat=""+meat.name+'_Pic'+"";
  meat.addEventListener("click",changeColor(idNumberMeat), false);
}
...
for (var i = 0; i < buns.length; ++i) {
  ...
  bun.addEventListener("click", selectBun, false);
    var idNumberBun=""+bun.name+'_Pic'+"";
    bun.addEventListener("click",changeColor(idNumberBun), false);
}
...
for (var i = 0; i < cheeses.length; ++i) {
  ...
  cheese.addEventListener("click", selectCheese, false);
    ...
    var idNumberCheese=""+cheese.name+'_Pic'+"";
    cheese.addEventListener("click",changeColor(idNumberCheese), false);
...
for (var i = 0; i < sides.length; ++i) {
  ...
  side.addEventListener("click", selectSide, false);
    ...
    var idNumberSide=""+side.name+'_Pic'+"";
    side.addEventListener("click",changeColor(idNumberSide), false);
...
for (var i = 0; i < toppings.length; ++i) {
  ...
  t.addEventListener("click", selectToppings, false);
    var idNumberT=""+t.name+'_Pic'+"";
    t.addEventListener("click",changeColor(idNumberT), false);
...
for (var i = 0; i < sauces.length; ++i) {
  ...
  sauce.addEventListener("click", selectSauces, false)
    var idNumberSauce=""+sauce.name+'_Pic'+"";
    sauce.addEventListener("click",changeColor(idNumberSauce), false);
...
function changeColor(idNumber) {
    alert(idNumber);
    //get the picture name of the image
    var x = document.getElementById(idNumber);
    alert('inside handloer 2' + x.src );
    var indexVar = x.src.indexOf('Color');
    if (indexVar > -1) {
        //select substring without color and add .png to end
        x.src = x.src.replace('_Color.jpg', '.png');
    }
    else {
        x.src = x.src.replace('.png', '_Color.jpg');
    }
